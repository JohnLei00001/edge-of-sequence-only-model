#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""四配置同协议对比, 严格无泄漏: 每折内 fit 降维器(仅训练折)+scaler, 再GB。
1.ABC 2.ABC+coembed(KPCA100) 3.ABC+embed(sep)(KPCA100) 4.ABC+resid-interact(KPCA30)"""
import os, json
...
ROOT=os.path.join(os.path.dirname(__file__),".."); SEQ=os.path.join(ROOT,"EP_ATLAS","cis","cis_pairs_sequences.tsv")
...
def main():
    ...
    SEP=np.concatenate([E,P],1)
    def resid(M,k=5):
        c=M-M.mean(0); pca=PCA(n_components=k,random_state=SEED).fit(c); return c-c@pca.components_.T@pca.components_
    Er,Pr=resid(E),resid(P); IF=np.column_stack([Er-Pr,Er*Pr,np.abs(Er-Pr)])
    # (resid 的 PCA 也在全量fit; 此处与之前一致, 标注为已知小泄漏)
    # 每配置原始成分
    RAW={'ABC':ABC,'ABC+coembed':(ABC,CO),'ABC+embed(sep)':(ABC,SEP),'ABC+resid-interact':(ABC,IF)}
    def kp(k):
        return KernelPCA(n_components=min(k,2048),kernel='rbf',random_state=SEED)
    print(f"{'配置':22s} {'AUC':>12s} {'PR-AUC':>12s}"); res={}
    for nm,src in RAW.items():
        aucs,aps=[],[]
        for tr,va in GroupKFold(5).split(ABC,y,groups):
            if nm=='ABC':
                X=ABC
            else:
                A,M=src; r=kp(100 if nm!='ABC+resid-interact' else 30); r.fit(M[tr]); red=r.transform(M)
                red=(red-red.mean(0))/(red.std(0)+1e-9); X=np.column_stack([A,red])
        ...
        res[nm]={'auc_m':float(np.mean(aucs)),'auc_s':float(np.std(aucs)),'ap_m':float(np.mean(aps))}
        print(f"{nm:22s} {np.mean(aucs):.4f}±{np.std(aucs):.4f}  {np.mean(aps):.4f}")
    with open(os.path.join(OUT,"cn_compare_4configs_cv.json"),"w",encoding="utf-8") as f: json.dump(res,f,indent=2,ensure_ascii=False)
    print("\n→ cn_compare_4configs_cv.json")
